...
####テンプレート作成上の注意点
広告箇所が広告であることをユーザーが視認・理解できる文言を必ず記述して下さい。
...
この設置方法では、広告スクリプトを非同期で読み込み、ページのメインコンテンツの描画完了後に広告の表示処理を開始します。メインコンテンツへの影響を最小限に
できます。一方でメインコンテンツの描画後に広告が挿入されることが起こりやすくなり、ページの表示に違和感が多くなる可能性があります。よって事前に表示テストを行うおとを推奨します。
